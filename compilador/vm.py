...
class VirtualMachine:
    '''Execute and process quads generated by the compiler'''
    ...
    def print_mem_tree(self):
        '''Print the current state of the ExecutionScopeTree'''
        print(self.mem_tree)
    ...
    def set_constants(self):
        '''Load constants from the constants_table to the Execution/mememory tree
        at the start of the execution. Loads them into th inital/global/root Execution Node'''
    ...
    def execute_quads(self, run_in_terminal=False):
        '''Execute Quads from the quad_list until the END quad is processed'''
        if run_in_terminal:
    ...
    def execute_quads_in_terminal(self):
        '''Execute Quads and manage input and output on the terminal'''
        while True:
            self.set_current_quad()
            op_code = self.current_quad.op_code
            self.execute_op(op_code)
    ...
    def get_value(self, addr, check_for_virtual=True):
        '''Get the value of a specified address from the Execution/Memory tree
        If the address is virtual, get the value for the address 2 consecutive times'''
    ...
    def set_value(self, addr, result, check_for_virtual=False):
        '''Set the value of a specified address in the Execution/Memory tree.
        If the address is virtual, get the real address first and then set.'''
        if check_for_virtual and addr in self.virtual_var_list:
